...
import { linkTo, scrollTo } from "@/utils/jump";
export default {
  data() {
    return {
      color: [
  ...
  methods: {
    ...
    async getContent() {
      await this.$http.Home.getHotInfo()
        .then((e) => {
          if (e.data.code === "1") {
        ...
        .catch((error) => {
          this.tip({ type: "fail", message: "热门内容获取失败！" });
        });
    },
    gotoAnotherModule(item) {
      if (this.timer) {
